...
public class LongStringSplit {
	...
	public static void main(String[] args) {
		...
		//1. 전체 분리된 단어의 갯수
		//장문(String)을 split("빈칸")함수로 잘라서 String[] 배열에 담는다. 
		//빈칸으로 구분된 단어 갯수만큼 배열 갯수가 만들어짐
		String[] strArr = str.split(" ");	//문자열을 (" ")로 구분해서 자르고 그걸 문자열 배열에 담아서 반환
		...
		//3. 위의 str 데이터 중 "HTML5"과 "하이브리드앱"라는 단어가 몇번 노출되는 지
		//   노출 횟수를 resultNum이라는 배열명으로 저장하여 출력
		int count = 0;
		 for (String string : strArr) {
			 if(string.equals("HTML5") || string.equals("하이브리드앱")) {
				 count++;
			 }
		}
		System.out.println("1. HTML5와 하이브리드앱 단어는 총 : " + count + "회 노출되었습니다.");
		System.out.println();
		
		//문제4. 위의 str 데이터를 모두 역순(space기준)으로 
